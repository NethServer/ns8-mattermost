...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-mattermost; then
    ...
    buildah from --name nodebuilder-mattermost -v "${PWD}:/usr/src:Z" docker.io/node:24.14.1-alpine
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/postgres:17.9-alpine docker.io/mattermost/mattermost-team-edition:10.11.14" \
    "${container}"
# Commit the image
