...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-mattermost; then
    ...
    buildah from --name nodebuilder-mattermost -v "${PWD}:/usr/src:Z" docker.io/node:24.15.0-alpine
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/postgres:17.10-alpine docker.io/mattermost/mattermost-team-edition:10.11.17" \
    "${container}"
# Commit the image
