...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-mattermost; then
    ...
    buildah from --name nodebuilder-mattermost -v "${PWD}:/usr/src:Z" docker.io/node:18.14.2-alpine
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/postgres:13.10-alpine docker.io/mattermost/mattermost-team-edition:$MATTERMOST_VERSION" \
    "${container}"
# Commit the image
